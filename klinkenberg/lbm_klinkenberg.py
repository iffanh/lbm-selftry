...
for t in range(ini.T):
    ...
    #OK
    ini.ux[1,:] = -1. + ((ini.f[1,:,0] + ini.f[1,:,2] + ini.f[1,:,4] + 2.*(ini.f[1,:,3] + ini.f[1,:,7] + ini.f[1,:,6])) / ini.rho_left)
    ini.ru[:] = -ini.rho_left*ini.ux[1,:]
    ini.f[1,:,1] = (ini.f[1,:,3] + (2./3.)*ini.ru[:])
    ini.f[1,:,5] = (ini.f[1,:,7] + (1./6.)*ini.ru[:] - (1./2.)*(ini.f[1,:,2] - ini.f[1,:,4]))
    ini.f[1,:,8] = (ini.f[1,:,6] + (1./6.)*ini.ru[:] - (1./2.)*(ini.f[1,:,4] - ini.f[1,:,2]))

    #Pressure BC on the right
    #OK
    ini.ux[1,:] = -1. + ((ini.f[-2,:,0] + ini.f[-2,:,2] + ini.f[-2,:,4] + 2.*(ini.f[-2,:,1] + ini.f[-2,:,5] + ini.f[-2,:,8])) / ini.rho_right)
    ini.ru = ini.rho_right*ini.ux[1,:]
    ini.f[-2,:,3] = (ini.f[-2,:,1] - (2./3.)*ini.ru)
    ini.f[-2,:,7] = (ini.f[-2,:,5] - (1./6.)*ini.ru + (1./2.)*(ini.f[-2,:,2] - ini.f[-2,:,4]))
    ini.f[-2,:,6] = (ini.f[-2,:,8] - (1./6.)*ini.ru + (1./2.)*(ini.f[-2,:,4] - ini.f[-2,:,2]))
    ...
    for j in range(1,ini.sizeY_+1):
        ...
        for i in range(1,ini.sizeX_+ 1):
            ...
            if (ini.m[i,j] <> 1):
                ...
                else:                      ini.ftemp[i,j,7] += alpha*ini.f[i,j,5]; ini.ftemp[i,j,8] += (1-alpha)*ini.f[i,j,5]

                if (ini.m[i_n,j_p] <> 1):  ini.ftemp[i_n,j_p,6] = ini.f[i,j,6]
                else:                      ini.ftemp[i,j,8] += alpha*ini.f[i,j,6]; ini.ftemp[i,j,7] += (1-alpha)*ini.f[i,j,6]

                if (ini.m[i_n,j_n] <> 1):  ini.ftemp[i_n,j_n,7] = ini.f[i,j,7]
                else:                      ini.ftemp[i,j,5] += alpha*ini.f[i,j,7]; ini.ftemp[i,j,6] += (1-alpha)*ini.f[i,j,7]

                if (ini.m[i_p,j_n] <> 1):  ini.ftemp[i_p,j_n,8] = ini.f[i,j,8]
                else:                      ini.ftemp[i,j,6] += alpha*ini.f[i,j,8]; ini.ftemp[i,j,5] += (1-alpha)*ini.f[i,j,8]
    ...
    #     #ax = sns.heatmap(varm, annot=False, vmin=0., vmax=1.5, cmap='RdYlBu_r', mask=ini.m.transpose())           #For rho
    ...
    if mod(t,10) == 0:
        plt.figure(2)
        plt.plot(ini.ux[-5,:],label='Outlet')
        plt.plot(ini.ux[(ini.sizeX_+2)//2,:],label='Middle')
        plt.plot(ini.ux[5,:],label='Inlet')
